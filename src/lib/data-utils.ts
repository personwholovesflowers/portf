...
export async function getAllMyProjects(): Promise<CollectionEntry<'myprojects'>[]> {
  const myprojects = await getCollection('myprojects')
...
export async function getPostsByMember(
  ...
  const posts = await getAllPosts()
  return posts.filter((post) => post.data.members?.includes(memberId))
}
...
export async function parseMember(memberIds: string[] = []) {
